...
const api_call = (method, data, wheel, lock_) => {
    lock = new Promise( async (resolve, reject) => {
        let url = 'http://192.168.5.134:8010/api_' + method
...
//fill dnameSel select
function fillDBName(dViewList)
{
  $('select#dtypeDB').empty(); //clear
  $('select#dtypeDB').append($('<option>', { value: "0", text : "-- не выбран --"}));
  $.each(dViewList, function (i, item) {
    if (typeof item !== "undefined") {
      $('select#dtypeDB').append($('<option>', { 
        value: item.code,
        text : item.name 
      }));
    }
...
//get matadata
let dataDB
dataDB = api_call('log_get_db', {}, true, true)
...
$(function() {
  ...
  $('select[name="dtypeDB"]').on('change', function() {
...
$(document).on("change",'form.lb-form input[name="dnum"]', function(e){
    ...
    //get event objs
    let dataEO
    dataEO = api_call('log_get_event_objs', {"db":$('select#dtypeDB option:selected').val(), "metadata_id" : $('form.lb-form select[name="dname"]').val(), "data_string" : $('form.lb-form input[name="dnum"]').val(), "date_start" : $('form.lb-form input[name="ddatestart"]').val(), "date_end" : $('form.lb-form input[name="ddateend"]').val()}, true, true)
